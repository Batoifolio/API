  ...
  public authenticate = (req: Request, res: Response, next: NextFunction): void => {
    try {
      ...
      if (authHeader === undefined) {
        return this.errorResponse(req, res, null, 'Token no proporcionado', 403)
      }

      // 2. Verificar que el formato sea "Bearer <token>"
      const parts = authHeader.split(' ')
      if (parts.length !== 2 || parts[0] !== 'Bearer') {
        return this.errorResponse(req, res, null, 'Formato de token inválido', 403)
      }
      ...
      jwt.verify(token, process.env.JWT_SECRET as string, (err, decoded) => {
        if (err != null) {
          return this.errorResponse(req, res, err, 'Token inválido', 403)
        }
      ...
      this.errorResponse(req, res, error, 'Error en la autenticación')
    }
  }
}
